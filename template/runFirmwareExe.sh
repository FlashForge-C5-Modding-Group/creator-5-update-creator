...
create_key()
{
    ...
    if [ ! -f ${key_pair} ];then
        ...
        openssl rsa -in ${key_pair} -passin pass:${password} -outform PEM
    fi
}
...
update_kernel()
{
    ...
    if [ $? -eq 0 ];then
        ...
        if [ $? -eq 0 ];then
            ...
            cd ${kernel_version}
            if [ -f run.sh ];then
                chmod a+x ${RUN_DIR}/kernel/${kernel_version}/run.sh
...
update_software()
{
    ...
    if [ $? -eq 0 ];then
        ...
        if [ $? -eq 0 ];then
            ...
            cd ${software_version}
            if [ -f run.sh ];then
                chmod a+x /${RUN_DIR}/software/${software_version}/run.sh
...
update_library()
{
    ...
    if [ $? -eq 0 ];then
        ...
        if [ $? -eq 0 ];then
            ...
            cd ${library_version}
            if [ -f run.sh ];then
                chmod a+x ${RUN_DIR}/library/${library_version}/run.sh
